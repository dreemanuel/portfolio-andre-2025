...
const Waves = ({
  lineColor = "black",
  gradientColors = null, // New prop for gradient: ["#33ccff", "#00ff99"]
  ...
  useEffect(() => {
    ...
    function drawLines() {
      ...
      if (
        configRef.current.gradientColors &&
        configRef.current.gradientColors.length >= 2
      ) {
        const gradient = ctx.createLinearGradient(0, 0, width, height);
        configRef.current.gradientColors.forEach((color, index) => {
          gradient.addColorStop(
            index / (configRef.current.gradientColors.length - 1),
            color,
          );
        });
